...
#required_plugins = %w(vagrant-env vagrant-git vagrant-proxyconf vagrant-persistent-storage)
...
#      kube.vm.network "public_network", ip: "#{$subnet}.#{kb}"
	  ...
	  # Attach additional disk
          ...
          kube.persistent_storage.mountpoint= $disk_mount
          kube.persistent_storage.location = $disk_location+"kube#{kb}/"+$disk_image_file
          kube.persistent_storage.size = $kube_disk * 1024 - 10240
          kube.persistent_storage.manage = true
          kube.persistent_storage.filesystem = 'ext4'
          #kube.persistent_storage.volgroupname = $disk_vol
          kube.persistent_storage.diskdevice = $disk_device
        end
      end
